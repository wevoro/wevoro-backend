 ...
 */
export function calculateProCompletion(
  personalInfo: any,
  ...
  if (personalInfo && Object.keys(personalInfo).length > 0) score += 25;
  if (professionalInfo && Object.keys(professionalInfo).length > 0) score += 25;
  ...
  const perCredential = 50 / REQUIRED_CREDENTIAL_KEYS.length;
  REQUIRED_CREDENTIAL_KEYS.forEach(key => {
    const doc = docsByType[key];
    if (!doc) return;
    score += perCredential / 2;
    if (doc.reviewStatus === 'approved') score += perCredential / 2;
  });
