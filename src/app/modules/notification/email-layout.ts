...
export const EMAIL_SANS = 'Arial,Helvetica,sans-serif';
 ...
 */
export const emailAppUrl = (): string =>
  (process.env.APP_PUBLIC_URL || 'https://wevoro.com').replace(/\/+$/, '');

export const emailAsset = (file: string): string => `${emailAppUrl()}/email/${file}`;
...
export const emailDetailRow = (label: string, value: string, topPad: number): string => `
...
export interface AlertEmailInput {
  /** Emblem PNG filename under public/email/, e.g. "emblem-yellow.png". */
  emblem: string;
  eyebrow: string;
  /** Eyebrow colour. */
  accent: string;
  heading: string;
  /** Already-escaped greeting name; rendered as "Hi {name}, {intro}". */
  greetingName: string;
  /** Intro sentence(s); may contain entities, appended after the greeting. */
  intro: string;
  /** Pre-rendered rows (emailDetailRow) for the grey card; empty hides it. */
...
export const renderAlertEmail = (p: AlertEmailInput): string => `<!doctype html>
<html><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>${escapeHtml(p.heading)}</title></head>
 ...
 <tr><td align="center">
  <table role="presentation" width="560" cellpadding="0" cellspacing="0" border="0"
    ...
    <tr><td style="background:${C.green};padding:24px 32px">
      <img src="${emailAsset('header-lockup.png')}" width="189" height="41"
    ...
    <tr><td style="padding:32px">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
        ...
        <tr><td style="padding:0 0 18px 0">
          <img src="${emailAsset(p.emblem)}" width="56" height="56" alt=""
        ...
        <tr><td style="padding:6px 0 0 0;font-family:${SANS};font-size:22px;line-height:30px;
            font-weight:bold;color:${C.heading}">${escapeHtml(p.heading)}</td></tr>
    ...
    <tr><td style="background:${C.pageBg};padding:22px 32px">
      <img src="${emailAsset('footer-lockup.png')}" width="137" height="34" alt="Wevoro - www.wevoro.com"
           style="display:block;border:0;outline:none;height:auto">
